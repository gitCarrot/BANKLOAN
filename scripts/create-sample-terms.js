...
If you have any questions about our use of cookies or other technologies, please email us at privacy@example.com.`,
  ...
  for (const termData of sampleTerms) {
    try {
      ...
      if (existingTerm) {
        console.log(`Term "${termData.name}" already exists with ID ${existingTerm.termsId}, updating content and version...`);
        
        // Update existing term with content and version
        await prisma.terms.update({
          where: { termsId: existingTerm.termsId },
          data: {
        ...
        console.log(`Updated term "${termData.name}" with content and version`);
        continue;
      }
      ...
      // Create term with all fields including content and version
      ...
      console.log(`Created term "${termData.name}" with ID ${term.termsId} including content and version`);
    } catch (error) {
      console.error(`Error creating/updating term "${termData.name}":`, error.message);
    }
  }
